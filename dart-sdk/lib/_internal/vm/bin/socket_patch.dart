  ...
  factory _InternetAddress.fixed(int id) {
    switch (id) {
      case _addressLoopbackIPv4:
        ...
        return _InternetAddress(
            InternetAddressType.IPv4, "127.0.0.1", null, inAddr);
      ...
      case _addressAnyIPv4:
        var inAddr = Uint8List(_IPv4AddrLength);
        return _InternetAddress(
            InternetAddressType.IPv4, "0.0.0.0", "0.0.0.0", inAddr);
...
base class _NativeSocket extends _NativeSocketNativeWrapper
    ...
    return _IOService._dispatch(_IOService.socketListInterfaces, [type._value])
      ...
      if (isErrorResponse(response)) {
        ...
        var map = (response as List)
            .skip(1)
            .fold(Map<String, NetworkInterface>(), (map, result) {
  ...
  static Future<_NativeSocket> bindDatagram(
    ...
    if (result is OSError) {
      throw SocketException("Failed to create datagram socket",
          osError: result, address: address, port: port);
    }
    if (port != 0) socket.localPort = port;
    return socket;
  ...
  external nativeCreateConnect(Uint8List addr, int port, int scopeId);
  ...
  external nativeCreateBindConnect(Uint8List addr, int port,
      Uint8List sourceAddr, int sourcePort, int scopeId);
  ...
  external nativeCreateBindListen(Uint8List addr, int port, int backlog,
      bool v6Only, bool shared, int scopeId);
...
class _ServerSocket extends Stream<Socket> implements ServerSocket {
  ...
  StreamSubscription<Socket> listen(void Function(Socket event)? onData,
      {Function? onError, void Function()? onDone, bool? cancelOnError}) {
    return _socket.map<Socket>((rawSocket) => _Socket(rawSocket)).listen(
        onData,
        onError: onError,
        onDone: onDone,
  ...
  static Future<ConnectionTask<Socket>> _startConnect(dynamic host, int port,
    ...
    return RawSocket.startConnect(host, port,
      ...
      Future<Socket> socket =
          rawTask.socket.then((rawSocket) => _Socket(rawSocket));
...
Datagram _makeDatagram(
    Uint8List data, String address, Uint8List inAddr, int port, int type) {
  return Datagram(
      data,
